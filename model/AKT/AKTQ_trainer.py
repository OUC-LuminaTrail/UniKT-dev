"""AKTQ trainer: question-level AKT for the skill-vs-question ablation."""
...
@register_model_config("AKTQ")
class AKTQConfig(ModelConfig):
    """AKTQ configuration.

    Same architecture as AKT, but the question id is the concept embedding
    unit (``num_c=num_questions``) and Rasch pid is disabled. Hyperparameters
    mirror AKT so the ablation varies only the modeling granularity.
...
@register_trainer("AKTQ")
class AKTQTrainer(BaseTrainer):
    """Question-level AKT trainer (ablation variant).

    Reuses the AKT model with question-level concept embeddings and no Rasch
    pid. Test follows the standard K-fold split instead of windowlate.
    """

    def build_components(self, rc, data_src) -> RuntimeComponents:
        from model.AKT.AKT_model import AKT
        from model.AKT.AKTQ_data import AKTQModelData

        model_data = AKTQModelData(data_src)
        train_dataset, val_dataset, test_dataset = model_data.prepare_data(rc)

        num_questions = data_src.get_metadata("num_questions")
        logger.info(
            f"Initializing AKTQ (question-level) with {num_questions} questions"
        )
    ...
    def forward_pass(
        ...
        matches AKT (same_position=True): ``y_hat[:, t]`` predicts
        ``response[t]``.
        """
