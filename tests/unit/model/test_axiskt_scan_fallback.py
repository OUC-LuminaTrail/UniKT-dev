...
def test_serial_fallback_post_multiply_matches_explicit_transition():
    matrix, bias, segment_ids, valid_mask, initial = _example()
    actual = segmented_scalar_affine_exclusive_scan(
        matrix,
        bias,
    ...
    expected = (
        _scalar_reference(matrix, bias, segment_ids, valid_mask, initial)
        * matrix[..., 0]
    )
    torch.testing.assert_close(actual, expected)
...
def test_coded_cpu_fallback_matches_reference():
    segment_ids = torch.tensor([[0, 0, 1, 1, 1, 2]], dtype=torch.long)
    valid_mask = torch.tensor([[1, 1, 1, 0, 1, 1]], dtype=torch.bool)
    table = 0.9 + 0.1 * torch.randn(3, 4)
    codes = torch.tensor([[0, 2, 1, 1, 0, 2]], dtype=torch.long)
    bias = torch.randn(1, 6, 4, 1)
    initial = torch.randn(1, 6, 4, 1)
    ...
    expected = _scalar_reference(dense, bias, segment_ids, valid_mask, initial)
    torch.testing.assert_close(actual, expected, rtol=1e-4, atol=1e-5)


def test_coded_cpu_fallback_gradients_match_dense():
    torch.manual_seed(5)
    segment_ids = torch.tensor([[0, 0, 1, 1, 1, 2]], dtype=torch.long)
    ...
    out_d = segmented_scalar_affine_exclusive_scan(
        matrix_d, bias, segment_ids, valid_mask, initial
    )
    (out_d * weights).sum().backward()

    expected_table_grad = torch.zeros_like(table).index_add_(
        0, codes.reshape(-1), matrix_d.grad.reshape(-1, 4)
    )
    torch.testing.assert_close(table_c.grad, expected_table_grad, rtol=1e-4, atol=1e-5)


def test_coded_scan_rejects_conflicting_inputs():
    bias = torch.randn(1, 4, 2, 1)
    initial = torch.randn(1, 4, 2, 1)
    segment_ids = torch.zeros(1, 4, dtype=torch.long)
    valid_mask = torch.ones(1, 4, dtype=torch.bool)
    matrix = torch.randn(1, 4, 2, 1, 1)
    table = torch.rand(2, 2)
    codes = torch.zeros(1, 4, dtype=torch.long)

    with pytest.raises(ValueError, match="matrix must be None"):
        segmented_scalar_affine_exclusive_scan(
            matrix,
            bias,
            segment_ids,
    ...
    with pytest.raises(ValueError, match="matrix_codes is required"):
        segmented_scalar_affine_exclusive_scan(
            None,
            bias,
            segment_ids,
...
def test_coded_init_cpu_fallback_matches_reference():
    segment_ids = torch.tensor([[0, 0, 1, 1, 1, 2]], dtype=torch.long)
    valid_mask = torch.tensor([[1, 1, 1, 0, 1, 1]], dtype=torch.bool)
    matrix = 0.9 + 0.1 * torch.randn(1, 6, 3, 1, 1)
    bias = torch.randn(1, 6, 3, 1)
    init_table = torch.randn(3, 3)
    init_codes = torch.tensor([[0, 2, 1, 1, 0, 2]], dtype=torch.long)
    ...
    expected = _scalar_reference(matrix, bias, segment_ids, valid_mask, dense_init)
    torch.testing.assert_close(actual, expected, rtol=1e-4, atol=1e-5)


def test_coded_init_cpu_fallback_gradients_match_dense():
    torch.manual_seed(7)
    segment_ids = torch.tensor([[0, 0, 1, 1, 1, 2]], dtype=torch.long)
    valid_mask = torch.ones(1, 6, dtype=torch.bool)
    matrix = 0.9 + 0.1 * torch.randn(1, 6, 3, 1, 1)
    ...
    out_d = segmented_scalar_affine_exclusive_scan(
        matrix, bias, segment_ids, valid_mask, init_d
    )
    (out_d * weights).sum().backward()

    expected_table_grad = torch.zeros_like(init_table).index_add_(
        0, init_codes.reshape(-1), init_d.grad.reshape(-1, 3)
    )
    torch.testing.assert_close(
...
def test_coded_init_rejects_conflicting_inputs():
    bias = torch.randn(1, 4, 2, 1)
    segment_ids = torch.zeros(1, 4, dtype=torch.long)
    valid_mask = torch.ones(1, 4, dtype=torch.bool)
    matrix = torch.randn(1, 4, 2, 1, 1)
    initial = torch.randn(1, 4, 2, 1)
    init_table = torch.rand(2, 2)
    init_codes = torch.zeros(1, 4, dtype=torch.long)

    with pytest.raises(ValueError, match="initial_state must be None"):
        segmented_scalar_affine_exclusive_scan(
            matrix,
            bias,
            segment_ids,
    ...
    with pytest.raises(ValueError, match="init_codes is required"):
        segmented_scalar_affine_exclusive_scan(
            matrix,
            bias,
            segment_ids,
    ...
    with pytest.raises(ValueError, match="initial_state is required"):
        segmented_scalar_affine_exclusive_scan(
            matrix, bias, segment_ids, valid_mask, None
        )
