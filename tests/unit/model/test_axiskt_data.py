...
class _KfoldQuestionDataSource:
    """Fake question-level DataSource with fold labels and a Q-S relation."""

    def __init__(self, df, max_seq_len):
        self.dataset = "fake_kfold"
        self._df = df
        self.relation = pl.DataFrame({"question": [0, 1, 2], "skill": [1, 0, 2]})
        self.metadata = {
...
def _kfold_split_frame():
    """User 7 spans two subsequences (rows 0, 1); folds 0/1/-1 split them."""
    return pl.DataFrame(
        {
            "user": [7, 7, 7, 7, 8, 8, 9],
            "sequence_id": [0, 0, 1, 1, 2, 2, 3],
            "seq_pos": [0, 1, 0, 1, 0, 1, 0],
            "question": [0, 1, 0, 2, 1, 0, 2],
            "label": [1, 0, 1, 1, 0, 1, 0],
            "fold": [0, 0, 0, 0, 1, 1, -1],
        }
    )
...
def test_prepare_data_reports_real_user_ids_per_split_row():
    """Split rows are subsequences and folds compress them; user ids must
    still come from the parquet's ``user`` column, never from row indices."""
    from types import SimpleNamespace

    from model.AxisKT.AxisKT_analyzer import _UserAwareAxisKTDataset

    data_src = _KfoldQuestionDataSource(_kfold_split_frame(), max_seq_len=2)
    rc = SimpleNamespace(data=SimpleNamespace(fold=0))

    _, val_data, _, extra = AxisKTModelData(data_src).prepare_data(rc)

    np.testing.assert_array_equal(extra["user_ids"]["train"], [8])
