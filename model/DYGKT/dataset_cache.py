...
def get_cache_dir(base_dir: str | Path | None = None) -> Path:
    ...
    """
    base_dir = Path("./cache/dygkt") if base_dir is None else Path(base_dir)
...
def load_cached_dataset(
    ...
    try:
        ...
        logger.info(
            f"✅ Cache hit: {cache_key} (loaded in {cache_file.stat().st_size / 1024 / 1024:.1f} MB)"
        )
        return cached_data
...
def save_cached_dataset(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            "wb", dir=cache_dir, suffix=".tmp", delete=False
        ) as f:
...
def clear_cache(cache_dir: str | Path | None = None, pattern: str = "*.pkl") -> int:
    ...
    if deleted_count > 0:
        logger.info(
            f"✅ Cleared {deleted_count} cache files ({total_size / 1024 / 1024:.1f} MB)"
        )
    else:
