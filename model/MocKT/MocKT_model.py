...
class FrequencyLayer(nn.Module):
    ...
    按真实序列长度将batch样本分为3组（短/中/长），每组使用独立kernel size的
    因果卷积作低通滤波，可学习 sqrt_beta^2 控制高通分量权重，处理完放回原位。
    """

    def __init__(self, dropout, hidden_size, kernel_size, sequence1, sequence2):
        super().__init__()
        self.out_dropout = nn.Dropout(dropout)
        self.layer_norm = nn.LayerNorm(hidden_size, eps=1e-12)
        # one causal conv per length group, with shrinking/growing kernels
        kernel_sizes = [kernel_size - 2, kernel_size, kernel_size + 2]
        self.causal_convs = nn.ModuleList(
            [CausalConv1d(hidden_size, hidden_size, ks) for ks in kernel_sizes]
        )
        self.sqrt_betas = nn.ParameterList(
            [nn.Parameter(torch.randn(1, 1, hidden_size)) for _ in kernel_sizes]
        )
        self.sequence1 = sequence1
        self.sequence2 = sequence2

    def forward(self, input_tensor, seq_lens):
        # input_tensor: [B, S, H]; seq_lens: [B] true (unpadded) sequence lengths
        group_masks = (
            seq_lens <= self.sequence1,
            (seq_lens > self.sequence1) & (seq_lens <= self.sequence2),
            seq_lens > self.sequence2,
        )
        sequence_emb_fft = torch.zeros_like(input_tensor)
        for group_mask, causal_conv, sqrt_beta in zip(
            group_masks, self.causal_convs, self.sqrt_betas, strict=True
        ):
            group = input_tensor[group_mask]
            low_pass = causal_conv(group.transpose(1, 2)).transpose(1, 2)
            high_pass = group - low_pass
            # boolean-index write keeps gradients routed to the selected rows
            sequence_emb_fft[group_mask] = low_pass + sqrt_beta**2 * high_pass
...
class Architecture(nn.Module):
    """MocKT架构：可选分组频率滤波层 + 双流Transformer（QA编码器和知识检索器）"""
    ...
    def __init__(
        ...
        if use_frequency_layer:
            self.filter_layer = FrequencyLayer(
                dropout, d_model, kernel_size, sequence1, sequence2
            )
    ...
    def forward(self, q_embed_data, qa_embed_data, pid_embed_data=None, seq_lens=None):
        y = qa_embed_data
        x = q_embed_data

        if self.use_frequency_layer:
            x = self.filter_layer(x, seq_lens)
            y = self.filter_layer(y, seq_lens)
...
class MocKT(nn.Module):
    ...
    Args:
        ...
        use_frequency_layer: 是否启用分组频率分解层
        use_kernel_bias: 是否启用Kerple log kernel位置偏置
        use_exp_decay: 是否启用单调指数衰减注意力
        kernel_size: 分组因果卷积基准核大小（三组分别为 kernel_size-2/-/+2）
        sequence1: 分组频率滤波的短/中序列长度分界
        sequence2: 分组频率滤波的中/长序列长度分界
    """
    ...
    def __init__(
        ...
        super().__init__()
        # kernel_size - 2 must stay a valid conv kernel (>= 1)
        assert kernel_size >= 3, "kernel_size must be >= 3"
        assert 0 <= sequence1 < sequence2, (
            "sequence bounds must satisfy 0 <= sequence1 < sequence2"
        )
    ...
    def forward(
        ...
        d_output = self.model(
            q_embed_data, qa_embed_data, pid_embed_data, seq_lens=seq_lens
        )
