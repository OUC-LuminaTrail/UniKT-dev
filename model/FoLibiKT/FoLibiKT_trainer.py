...
@register_model_config("FoLibiKT")
class FoLibiKTConfig(ModelConfig):
    """FoLibiKT model configuration.
...
@register_trainer("FoLibiKT")
class FoLibiKTTrainer(BaseTrainer):
    """FoLibiKT 模型训练器

    负责初始化FoLibiKT模型、优化器和训练数据，并实现前向传播逻辑。
    ...
    def build_components(self, rc, data_src) -> RuntimeComponents:
        from model.FoLibiKT.FoLibiKT_data import FoLibiKTModelData

        model_data = FoLibiKTModelData(data_src)
        train_dataset, val_dataset, test_dataset = model_data.prepare_data(rc)

        from model.FoLibiKT.FoLibiKT_model import FoLibiKT

        logger.info("Initializing FoLibiKT model...")
        metadata = data_src.get_metadata()
        n_pid = metadata.get("num_questions", 0) if rc.model.use_rasch else 0

        if n_pid > 0:
            logger.info(
                f"FoLibiKT: Using Problem ID (Rasch model) with {n_pid} questions"
            )
        else:
            logger.info("FoLibiKT: Using skill-only model (Rasch disabled)")
