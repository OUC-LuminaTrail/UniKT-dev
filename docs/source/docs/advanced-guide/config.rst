...
**注意：** ``--deterministic`` 默认开启（\ ``True``\ ），会调用 ``torch.use_deterministic_algorithms`` 确保可复现性。若某个算子不支持 deterministic 模式导致训练失败，可以用 ``--no-deterministic`` 关闭。
