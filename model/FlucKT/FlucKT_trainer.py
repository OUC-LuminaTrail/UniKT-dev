...
@register_model_config("FlucKT")
class FlucKTConfig(ModelConfig):
    """FlucKT model configuration.
    ...
    Args:
        ...
        use_frequency_layer: Whether to enable the causal-conv frequency layer.
        ...
        kernel_size: Kernel size of the causal convolution.
...
@register_trainer("FlucKT")
class FlucKTTrainer(BaseTrainer):
    """FlucKT 模型训练器

    负责初始化FlucKT模型、优化器和训练数据，并实现前向传播逻辑。
    ...
    def build_components(self, rc, data_src) -> RuntimeComponents:
        from model.FlucKT.FlucKT_data import FlucKTModelData
        from model.FlucKT.FlucKT_model import FlucKT

        model_data = FlucKTModelData(data_src)
        train_dataset, val_dataset, test_dataset = model_data.prepare_data(rc)

        logger.info("Initializing FlucKT model...")
        metadata = data_src.get_metadata()
        n_pid = metadata.get("num_questions", 0) if rc.model.use_rasch else 0

        if n_pid > 0:
            logger.info(
                f"FlucKT: Using Problem ID (Rasch model) with {n_pid} questions"
            )
        else:
            logger.info("FlucKT: Using skill-only model (Rasch disabled)")
    ...
    def _build_pid_data(
        ...
        """Build FlucKT Rasch pid data with 0 reserved for padding."""
