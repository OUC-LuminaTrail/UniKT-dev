"""Trainer for HGIKT_QuestionSkillOnly variant."""
...
@register_model_params("HGIKT_QuestionSkillOnly")
class HGIKTQuestionSkillOnlyModelParams(BaseParamConfig):
    """HGIKT_QuestionSkillOnly model parameters - inherits from HGIKT."""

    def define_params(self) -> tuple[str, dict]:
        group_name = "HGIKT_QuestionSkillOnly Parameters"
        params = {
            "hidden_dim": {
                "type": int,
                "default": 250,
...
@TRAINERS.register("HGIKT_QuestionSkillOnly")
class HGIKTQuestionSkillOnlyTrainer(BaseTrainer):
    """Trainer for HGIKT with question-skill edges only.

    Uses custom data preparation (HGIKTQuestionSkillOnlyData) to build
    hetero_graph with only question-skill edges.
    """
    ...
    def __init__(
        ...
        from model.HGIKT.variants.hgikt_question_skill_only_data import (
            HGIKTQuestionSkillOnlyData,
        )

        model_data = HGIKTQuestionSkillOnlyData(data_src)
        ...
        from model.HGIKT.variants.hgikt_question_skill_only import (
            HGIKT_QuestionSkillOnly,
        )

        logger.info("Initializing HGIKT_QuestionSkillOnly model...")
        model = HGIKT_QuestionSkillOnly(
        ...
        self.with_training(
            ...
            hyperparams=args,
            model_name="HGIKT_QuestionSkillOnly",
