...
update. The analyzer captures those states during the forward pass (via a
few temporary hooks on :meth:`AxisKT._pack_kc_occurrences`,
:meth:`AxisKT._packed_question_conditioned_readout`,
:meth:`AxisKT._global_history_states` and the ``local_write`` module,
...
class _UserAwareAxisKTDataset(Dataset):
    """AxisKTDataset wrapper that exposes the dataset row index as user id.

    Split data rows are indexed by user id (contiguous 0..N-1), so the row
    index of the validation slice is the real user id. The extra trailing
    element is transparent to :func:`axiskt_packed_collate_fn`.
    """

    def __init__(self, dataset: AxisKTDataset):
...
class AxisKTAnalyzer(BaseCaseAnalyzer):
    ...
    def __init__(self, rc, data_src: DataSource, checkpoint_path: str):
        ...
        val_dataset = _UserAwareAxisKTDataset(val_data)
        self.configure_inference(
            rc,
            val_dataset,
            rc.model.batch_size,
            collate_fn=_analyzer_collate_fn,
            device=rc.general.device,
        ).build()
    ...
    def forward_pass(
        ...
        """
        users, questions, responses, times, mask, kc_order, valid_idx = batch_data
    ...
    def _forward_with_captured_states(
        ...
        Captures: the packed per-KC local states (from
        :meth:`AxisKT._packed_question_conditioned_readout`), the packed
        occurrence layout (from :meth:`AxisKT._pack_kc_occurrences`), and
        the local write ``bias`` (forward hook on ``local_write``, before
        the outer ``tanh``). The hooks are installed only around this call
        and removed in a ``finally`` block, so the analyzer never mutates
        the model's methods outside the forward pass.
        """
        captured: dict[str, torch.Tensor] = {}
        model = self.model
        orig_occurrences = model._pack_kc_occurrences
        orig_readout = model._packed_question_conditioned_readout
        ...
        def hooked_occurrences(
            q, r, t, m, skill_ids=None, skill_mask=None, kc_order=None
        ):
            out = orig_occurrences(q, r, t, m, skill_ids, skill_mask, kc_order)
            captured["packed_skill"] = out[0].detach()
            captured["packed_valid"] = out[4].detach()
            captured["packed_pos"] = out[6].detach()
            return out
        ...
        def hooked_readout(
            ...
            captured["packed_state"] = packed_state.detach().float()
            return orig_readout(
                packed_state,
                packed_skill_embedding,
                packed_pos,
        ...
        model._pack_kc_occurrences = hooked_occurrences
        model._packed_question_conditioned_readout = hooked_readout
        try:
            ...
            model._pack_kc_occurrences = orig_occurrences
            model._packed_question_conditioned_readout = orig_readout
            write_hook.remove()
    ...
    def extract_case_data(self, batch_data: Any, outputs: dict) -> dict:
        ...
        """
        users, questions, responses, times, mask, kc_order, _ = batch_data
