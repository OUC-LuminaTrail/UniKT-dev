...
class MCSKTConfig(ModelConfig):
    ...
    # powers of two so d_model % num_heads == 0 for every combination
    d_model: int = field(
        default=256,
    ...
    num_sgap: int = 100
    num_pcount: int = 15
    # non-overlapping ranges keep delta1 < delta2 for every combination
    delta1: float = field(
        default=0.25,
    ...
    delta2: float = field(
        default=0.667,
        metadata={"optuna": {"type": "float", "low": 0.5, "high": 0.8}},
    )
    epochs: int = 200
