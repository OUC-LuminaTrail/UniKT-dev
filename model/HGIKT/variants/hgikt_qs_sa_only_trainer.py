"""Trainer for HGIKT_QS_SA_Only variant."""
...
@register_model_params("HGIKT_QS_SA_Only")
class HGIKTQSSAOnlyModelParams(BaseParamConfig):
    """HGIKT_QS_SA_Only model parameters."""

    def define_params(self) -> tuple[str, dict]:
        group_name = "HGIKT_QS_SA_Only Parameters"
        params = {
...
@TRAINERS.register("HGIKT_QS_SA_Only")
class HGIKTQSSAOnlyTrainer(BaseTrainer):
    """Trainer for HGIKT with QS and SA edges only."""
    ...
    def __init__(
        ...
        # 1. Prepare data
        from model.HGIKT.variants.hgikt_qs_sa_only_data import HGIKTQSSAOnlyData
        model_data = HGIKTQSSAOnlyData(data_src)
        ...
        # 2. Initialize model
        from model.HGIKT.variants.hgikt_qs_sa_only import HGIKT_QS_SA_Only
        logger.info("Initializing HGIKT_QS_SA_Only model...")
        model = HGIKT_QS_SA_Only(
        ...
        self.with_training(epochs=args.epochs, seed=args.seed, device=args.device, checkpoint_path=args.checkpoint_path) \
            ...
            .with_experiment(exp_manager=exp_manager, hyperparams=args, model_name="HGIKT_QS_SA_Only", dataset_name=getattr(args, "dataset", "")) \
            .build()
