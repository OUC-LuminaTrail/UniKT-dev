...
class FoLibiKTModelData(SkillModelData):
    """FoLibiKT 模型数据加载器

    负责处理数据的预处理和加载，为FoLibiKT模型提供训练和验证数据。
    ...
    def prepare_data(self, rc: Any) -> tuple:
        ...
        logger.debug(
            f"FoLibiKT data prepared: train={len(train_dataset)}, val={len(val_dataset)}, test(window)={len(test_dataset)}"
        )
