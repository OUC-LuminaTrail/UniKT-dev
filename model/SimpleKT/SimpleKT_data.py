...
class SimpleKTModelData(SkillModelData):
    """SimpleKT 模型数据加载器

    负责处理数据的预处理和加载，为 SimpleKT 模型提供训练和验证数据。
    ...
    def prepare_data(self, args: Any) -> tuple:
        ...
        logger.debug(
            f"SimpleKT data prepared: train={len(train_dataset)}, val={len(val_dataset)}, "
