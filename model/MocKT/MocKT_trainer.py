...
@register_model_config("MocKT")
class MocKTConfig(ModelConfig):
    """MocKT model configuration.
    ...
    Args:
        ...
        use_frequency_layer: Whether to enable the grouped causal-conv frequency layer.
        use_kernel_bias: Whether to enable the Kerple log kernel position bias.
        use_exp_decay: Whether to enable monotonic exponential attention decay.
        kernel_size: Base kernel size of the grouped causal convolutions.
        sequence1: Length boundary between the short and medium groups.
        sequence2: Length boundary between the medium and long groups.
    ...
    # default matches the paper's qid_conv_ker_noexp variant
    use_exp_decay: bool = False
    # kernel_size - 2 must stay a valid conv kernel, see MocKT.__init__
    kernel_size: int = field(
        default=4,
        metadata={"optuna": {"type": "categorical", "choices": [4, 8, 16, 32]}},
    )
    # length bounds must satisfy 0 <= sequence1 < sequence2, see MocKT.__init__
    sequence1: int = field(
        default=50,
        metadata={"optuna": {"type": "categorical", "choices": [0, 20, 50, 70, 90]}},
    )
    sequence2: int = field(
        default=150,
        metadata={"optuna": {"type": "categorical", "choices": [130, 150, 170, 200]}},
    )
    epochs: int = 150
...
@register_trainer("MocKT")
class MocKTTrainer(BaseTrainer):
    """MocKT 模型训练器

    负责初始化MocKT模型、优化器和训练数据，并实现前向传播逻辑。
    ...
    def build_components(self, rc, data_src) -> RuntimeComponents:
        from model.MocKT.MocKT_data import MocKTModelData
        from model.MocKT.MocKT_model import MocKT

        model_data = MocKTModelData(data_src)
        train_dataset, val_dataset, test_dataset = model_data.prepare_data(rc)

        logger.info("Initializing MocKT model...")
        metadata = data_src.get_metadata()
        n_pid = metadata.get("num_questions", 0) if rc.model.use_rasch else 0

        if n_pid > 0:
            logger.info(f"MocKT: Using Problem ID (Rasch model) with {n_pid} questions")
        else:
            logger.info("MocKT: Using skill-only model (Rasch disabled)")
        ...
        model = MocKT(
            ...
            l2=m.l2,
            use_frequency_layer=m.use_frequency_layer,
            use_kernel_bias=m.use_kernel_bias,
            use_exp_decay=m.use_exp_decay,
    ...
    def _build_pid_data(
        ...
        """Build MocKT Rasch pid data with 0 reserved for padding."""
