...
class MocKTModelData(SkillModelData):
    """MocKT 模型数据加载器
    ...
    def prepare_data(self, rc: Any) -> tuple:
        ...
        logger.debug(
            f"MocKT data prepared: train={len(train_dataset)}, val={len(val_dataset)}, test(window)={len(test_dataset)}"
        )
