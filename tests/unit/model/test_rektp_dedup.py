...
def golden():
    ...
    return torch.load(_FIXTURE_PATH, weights_only=False)
...
def test_forward_matches_golden(golden):
    ...
    logits = model(
        golden["questions"], golden["responses"], golden["times"], golden["mask"]
    )
    # The refactor is a pure equivalence (shared tensors), so the output must
    # match bit-for-bit, not just within tolerance.
    torch.testing.assert_close(logits, golden["logits"], rtol=0, atol=0)
...
def test_backward_gradients_match_golden(golden):
    ...
    for name, param in model.named_parameters():
        ...
        torch.testing.assert_close(param.grad, golden_grad, rtol=1e-5, atol=1e-6)
...
def test_forward_dedups_question_derived_lookups(golden):
    ...
    model(golden["questions"], golden["responses"], golden["times"], golden["mask"])
