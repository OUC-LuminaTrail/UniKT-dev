...
class SparseKTModelData(SkillModelData):
    """SparseKT 模型数据加载器

    负责处理数据的预处理和加载，为 SparseKT 模型提供训练和验证数据。
    ...
    def prepare_data(self, rc: Any) -> tuple:
        ...
        train_dataset = SparseKTDataset(
            train_data[0], train_data[1], train_data[2], train_data[3]
        )
        val_dataset = SparseKTDataset(
        ...
        logger.debug(
            f"SparseKT data prepared: train={len(train_dataset)}, val={len(val_dataset)}, "
